...
- [ ] Task 1: Provision and wire real Google/GitHub OAuth credentials (AC: #1, #2, #3, #4)
  ...
  - [ ] Set real `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` in `.env.local`, Vercel env vars, and GitHub Actions secrets — all four env var names already exist in `.env.example`/`lib/auth/config.ts` from Story 1.0, only the values are currently placeholder — **PARTIAL**: `.env.local` done; Vercel/GitHub Actions secrets not yet confirmed
...
- [ ] Task 6: End-to-end verification (AC: all)
  - [ ] AC #1: no prior account, sign in with Google (or GitHub) → new `user` row created, `emailVerified` matches the provider's claim, signed in, redirected — confirm via `db:studio` or a direct query that exactly one `user` + one `account` (providerId=google) row exist
  - [ ] AC #2: create a normal email/password account, click its verification link (Story 1.1 flow), then sign in via Google/GitHub with the same email → still exactly **one** `user` row; a second `account` row (providerId=google) is now linked to it; signed into that same account
  - [ ] AC #3: create an email/password account and deliberately **do not** verify it, then attempt Google/GitHub sign-in with that same email → sign-in is rejected (redirected to `errorCallbackURL` with `?error=account_not_linked`), the UI shows the mapped message, and confirm no new/duplicate `user` row was created and the original unverified account is untouched
  - [ ] AC #4: spot-check a social-created account has the same shape (no role columns yet, per Story 1.1's precedent) as a password-created one
...
- 2026-08-20: Pushed to `main` to deploy to production (https://sanabel-six.vercel.app) so Ahmed can test social login end-to-end there. Vercel's project env vars still need the same four OAuth credential values set for it to work in production — flagged to Ahmed, not yet confirmed done.
...
- Task 1's credential provisioning is done (Ahmed, real Google/GitHub OAuth apps + `.env.local`). Task 6's live AC #1/#2/#3 walkthroughs (real OAuth round trips) are still pending Ahmed's manual click-through — cannot be done by the agent since it requires entering Ahmed's own account credentials. A verification script was used ad hoc to inspect `user`/`account` table state per email but was not committed.
